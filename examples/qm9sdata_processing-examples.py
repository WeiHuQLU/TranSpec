...
def loader(path1, path2, dictionary=None, shift=None, scale=None, quantize=None, seed=None):
    ...
    if shift or scale or quantize:
        ...
        torch.save(train_data, f"qm9s_train_irdata-examples{file_suffix}.pt")
    else:
        torch.save(train_data, "qm9s_train_irdata-examples.pt")

    if not (shift or scale or quantize):
        valid_data = MyDataSet(valid_spectrum, valid_smiles_idxs)
        torch.save(valid_data, "qm9s_valid_irdata-examples.pt")
        del valid_data
        
        test_data = MyDataSet(test_spectrum, test_smiles_idxs)
        torch.save(test_data, "qm9s_test_irdata-examples.pt")
        del test_data

    return dic

path1=r"qm9s_SMILES_5000.txt"
path2=r"qm9s_irdata_5000.csv"
