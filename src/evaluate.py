...
test_data = torch.load(r"qm9s_test_irdata.pt")
...
model.load_state_dict(torch.load(r"optimal_qm9s_ircnn_no-augmentation.pt", map_location=device))
model.eval()
...
def evaluate_model(model, test_loader, dic, dic_2, threshold_value, candidate_limit, output_prefix):
    ...
    label_file = f"{output_prefix}_label_ircnn.csv"
    candidate_file = f"{output_prefix}_candidate_ircnn.csv"
...
# Run the evaluation
evaluate_model(model, test_loader, dic, dic_2, threshold_value=0.01, candidate_limit=500, output_prefix="0.01")

# Calculate and print top-N accuracies
calculate_top_n_accuracy("0.01_candidate_ircnn.csv", "0.01_label_ircnn.csv", [1, 3, 5, 10])
